class AlarmClock {
  ...
  addClock(time, callback, id) {
    if (typeof id === "undefined") {
      ...
      typeof this.alarmCollection.find((clock) => clock.id === id) !==
      "undefined"
    ) {
  ...
  printAlarms() {
    return this.alarmCollection.forEach((clock) => {
      console.log(`Номер будильника: ${clock.id}, Время звонка: ${clock.time}`);
    });
  }
...
function testCase() {
  ...
  alarmClock.addClock("8:30", () => console.log("Просыпайся!", 1));
  alarmClock.addClock("8:31", () => console.log("Не спать!!!", 2));
  alarmClock.removeClock(2);
  alarmClock.addClock("8:32", () => console.log("Опаздываешь!", 3));
  alarmClock.stop();
